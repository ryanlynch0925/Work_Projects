...
from config import sent_back_CC, sent_back_sheet_name, sent_back_subject, removed_subject, correction_notes, signature, initialize_outlook, gather_corrections_data, cancelled_subject, amazon_invoice_error, walmart_dot_com_invoice_error, cheatsheet_path, itemization_instructions_path, globalIndustrial
        ...
        if correction is not None and not pd.isna(correction) and not pd.isna(date):
            ...
            if type(amount) == str:
                ...
                if correction == "Amazon Invoice Error" and not amazon_attached:
                    outlook_email.Attachments_Add(amazon_invoice_error)
                    amazon_attached = True
                elif correction == "Walmart.com Invoice Error" and not walmart_attached:
                    outlook_email.Attachments_Add(walmart_dot_com_invoice_error)
                ...
                if correction == "Amazon Invoice Error" and not amazon_attached:
                    outlook_email.Attachments.Add(amazon_invoice_error)
                    amazon_attached = True
                elif correction == "Walmart.com Invoice Error" and not walmart_attached:
                    outlook_email.Attachments.Add(walmart_dot_com_invoice_error)
                    walmart_attached = True
                elif (correction == "Itemization Needed" or correction == "Recheck Itemization") and not cheatsheet_attached and not itemization_attached:
                    outlook_email.Attachments.Add(cheatsheet_path)
                    outlook_email.Attachments.Add(itemization_instructions_path)
    ...
    for idx, row in group.iterrows():
        ...
        if correction is not None and not pd.isna(correction) and not pd.isna(date):
            ...
            if correction == "Global Industrial" and not globalIndustrial_attached:
                            ...
                            outlook_email.Attachments.Add(globalIndustrial)
